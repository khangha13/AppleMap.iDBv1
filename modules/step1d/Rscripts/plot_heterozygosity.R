...
if (length(args) < 2) {
  stop("Usage: Rscript plot_heterozygosity.R <metrics_tsv> <output_dir> [image_format]", call. = FALSE)
}
...
required_cols <- c("CHROM", "POS", "HETEROZYGOUS_RATE")
...
for (chrom in chromosomes) {
  chr_data <- metrics_dt[CHROM == chrom & !is.na(HETEROZYGOUS_RATE)]
  if (nrow(chr_data) == 0) {
    next
  }

  chr_data[, BIN := floor(POS_MBP / 0.5)]  # 0.5 Mbp bins
  window_stats <- chr_data[, .(
    pos_mid = mean(POS_MBP, na.rm = TRUE),
    het_mean = mean(HETEROZYGOUS_RATE, na.rm = TRUE),
    het_sd = sd(HETEROZYGOUS_RATE, na.rm = TRUE)
  ), by = BIN]
  window_stats[is.na(het_sd), het_sd := 0]
  if (nrow(window_stats) == 0) {
    next
  }
  ...
  plot_obj <- ggplot(window_stats, aes(x = pos_mid, y = het_mean)) +
    geom_ribbon(aes(ymin = pmax(het_mean - het_sd, 0),
                    ymax = pmin(het_mean + het_sd, 1)),
                fill = "#b2df8a", alpha = 0.35, colour = NA) +
    geom_line(color = "#33a02c", linewidth = 0.6) +
    scale_x_continuous(limits = c(min_pos, max_pos), breaks = breaks) +
    scale_y_continuous(labels = percent_format(accuracy = 1), limits = c(0, 1)) +
    labs(
      title = sprintf("Heterozygosity Across %s", chrom),
      x = "Position (Mbp)",
      y = "Mean Heterozygosity (per 0.5 Mbp)"
    ) +
  ...
  output_path <- file.path(output_dir, sprintf("%s_heterozygosity.%s", chrom, image_format))
