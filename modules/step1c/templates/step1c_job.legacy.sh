...
while IFS= read -r vcf_path; do
    ...
    if ! "${BCFTOOLS_BIN}" index -f -c "${clean_path}"; then
        log_warn "Failed to index ${clean_base}"
    fi
    ...
    if ! "${BCFTOOLS_BIN}" index -f -c "${beagle_path}"; then
        log_warn "Failed to index ${beagle_base}"
    fi
